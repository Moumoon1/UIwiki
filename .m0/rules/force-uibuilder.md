...
在执行任何 Figma 设计稿生成、编辑或更新任务之前，必须强制读取并加载本地 `uibuilder` 专属 Skill（即当前工作区 .m0/skills/下的 `SKILL.md` 文件）。

## 触发条件
- 任何涉及“生成设计稿”、“创建设计稿”、“编辑/更新设计稿”、“1:1还原/复刻设计稿”的指令。
- 任何准备调用 `m0_figma`（如 `html_write`、`html_sync`）工具或 `use_figma` 工具进行画布写入的场景。

## 强制执行步骤
1. **优先读取 Skill**：在开始构思设计或编写任何 m0-design 协议代码之前，必须先读取当前工作区根目录下的 `SKILL.md`（`/Users/jinhui/Desktop/UIbuilder/SKILL.md`）全文。
2. **遵循 UIbuilder 规范**：
